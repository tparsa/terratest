...
- If your script is used in a Packer template, add a [Docker
  ...
  same code. See the [Packer Docker Example](https://github.com/tparsa/terratest/tree/master/examples/packer-docker-example) for working sample code.

- We have prebuilt Docker images for major Linux distros that have many important dependencies (e.g., curl, vim,
  tar, sudo) already installed. See the [test-docker-images folder](https://github.com/tparsa/terratest/tree/master/test-docker-images) for more details.

- Create a `docker-compose.yml` to make it easier to run your Docker image with all the ports, environment variables,
  and other settings it needs. See the [Packer Docker Example](https://github.com/tparsa/terratest/tree/master/examples/packer-docker-example) for working sample code.
